...
$app->any('/finance/client/getall/plagroundID={playgroundID}', function (Request $request, Response $response) {
    ...
    $sth = $pdo->prepare('SELECT   id,barcodeID,name,data,(SELECT GROUP_CONCAT( name SEPARATOR \', \')FROM `product_list`, products WHERE product_list.barcodeID=products.barcodeID and clientID=client.id) as consumed,DATE_FORMAT(time,\'%T\') AS time, DATE_FORMAT(exitTime,\'%T\') AS exitTime,@t1:=floor(TIMESTAMPDIFF (minute,time,IF(exitTime=0,CURRENT_TIMESTAMP,exitTime))*10/60)  as price  ,@t2:=IFNULL((SELECT sum(-qty*price) as price FROM `product_list`,products WHERE clientID=client.id and product_list.barcodeID=products.barcodeID and product_list.playgroundID=client.playgroundID),0) as cost, @t1+@t2 as total_general FROM client     WHERE playgroundID= :playground and client.time>CURDATE()');
